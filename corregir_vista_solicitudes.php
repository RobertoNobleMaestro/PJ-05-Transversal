...
echo "=========== CORRIGIENDO VISTA DE SOLICITUDES ===========\n\n";

// 1. Identificar el problema potencial en la vista
$viewFile = __DIR__ . '/resources/views/chofers/solicitudes.blade.php';

if (file_exists($viewFile)) {
    echo "Archivo de vista encontrado: {$viewFile}\n";
    
    // Comprobar estructura del archivo
    $contenido = File::get($viewFile);
    
    // Buscar sección donde se maneja la carga
    $needle = 'id="loading-solicitudes"';
    if (strpos($contenido, $needle) !== false) {
        echo "Se encontró el div de carga en la vista.\n";
    } else {
        echo "ALERTA: No se encontró el div de carga en la vista.\n";
    }
    
    // Comprobar si la clase d-none está causando problemas
    if (strpos($contenido, 'class="d-none"') !== false) {
        echo "Se encontró la clase d-none en la vista, esto podría causar problemas de visualización.\n";
    }
    
    // Crear una copia de seguridad de la vista original
    File::copy($viewFile, $viewFile . '.backup');
    echo "Se ha creado una copia de seguridad de la vista original: {$viewFile}.backup\n\n";
    
    // Modificar la vista para asegurar que funcione correctamente
    $nuevaVista = str_replace('class="d-none"', 'style="display:block;"', $contenido);
    $nuevaVista = str_replace('<tbody>', '<tbody style="display:table-row-group;">', $nuevaVista);
    
    // Reemplazar el archivo JavaScript
    $scriptTag = '<script src="{{ asset(\'js/solicitudes.js\') }}"></script>';
    $nuevoScriptTag = '<script src="{{ asset(\'js/solicitudes_nuevo.js\') }}?v=' . time() . '"></script>';
    $nuevaVista = str_replace($scriptTag, $nuevoScriptTag, $nuevaVista);
    
    // Guardar los cambios
    File::put($viewFile, $nuevaVista);
    echo "Vista actualizada con éxito.\n\n";
} else {
    echo "ERROR: No se encontró el archivo de vista: {$viewFile}\n\n";
}

// 2. Crear una versión simplificada del JavaScript
$jsFile = __DIR__ . '/public/js/solicitudes_nuevo.js';
...
File::put($jsFile, $nuevoJs);
echo "Archivo JavaScript simplificado creado: {$jsFile}\n\n";

// 3. Verificar la ruta API
echo "Verificando ruta API...\n";
$routes = app()->router->getRoutes();
$encontrada = false;

foreach ($routes as $route) {
    if ($route->uri() == 'api/solicitudes/chofer') {
        $encontrada = true;
        echo "Ruta API encontrada: " . $route->getActionName() . "\n";
    }
}

if (!$encontrada) {
    echo "ALERTA: No se encontró la ruta API requerida.\n";
}

echo "=========== SOLUCIÓN COMPLETADA ===========\n";
echo "Por favor, actualiza el navegador para ver los cambios.\n";
