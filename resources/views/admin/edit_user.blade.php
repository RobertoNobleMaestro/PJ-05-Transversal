        ...
        <form id="editUserForm" data-url="/admin/users/{{ $user->id_usuario }}">
                ...
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="id_roles" name="id_roles">
                            @if ($user->id_roles == 1)
                            <option value="1" {{ $user->id_roles == 1 ? 'selected' : '' }}>Administrador</option>
                            @elseif($user->id_roles == 2 || $user->id_roles == 3)
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="licencia_conducir" name="licencia_conducir">
                            ...
                            @foreach($licencias as $licencia)
                                <option value="{{ $licencia }}" {{ $user->licencia_conducir == $licencia ? 'selected' : '' }}>
                                    {{ $licencia }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="btn-container">
                ...
                <button type="button" class="btn btn-submit"
                    onclick="updateUser({{ $user->id_usuario }})">Actualizar</button>
            </div>
        </form>
    </div>
@endsection
