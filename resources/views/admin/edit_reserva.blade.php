...
<div class="admin-container">
    ...
    <div class="admin-main">
        ...
        <div class="form-container">
            ...
            <form action="{{ route('admin.reservas.update', $reserva->id_reservas) }}" method="POST" id="reservaForm">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('fecha_reserva') is-invalid @enderror" id="fecha_reserva" name="fecha_reserva" value="{{ old('fecha_reserva', $reserva->fecha_reserva) }}" required>
                            @error('fecha_reserva')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('id_lugar') is-invalid @enderror" id="id_lugar" name="id_lugar" required>
                                ...
                                @foreach($lugares as $lugar)
                                    <option value="{{ $lugar->id_lugar }}" {{ old('id_lugar', $reserva->id_lugar) == $lugar->id_lugar ? 'selected' : '' }}>
                ...
                <div id="vehiculos-container" class="vehicles-container" data-count="{{ count($reserva_vehiculos) }}">
                    @foreach($reserva_vehiculos as $index => $vehiculo)
                        <div class="vehicle-entry" id="vehicle-entry-{{ $index }}">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        <h5 class="mb-0">Vehículo #{{ $index + 1 }}</h5>
                                        <button type="button" class="remove-vehicle-btn" onclick="removeVehicle({{ $index }})" {{ count($reserva_vehiculos) <= 1 ? 'style=display:none;' : '' }}>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-12">
                                    <label for="vehiculos_{{ $index }}" class="form-label">Seleccionar Vehículo</label>
                                    <select class="form-select" id="vehiculos_{{ $index }}" name="vehiculos[]" required onchange="calcularPrecio({{ $index }})">
                                        <option value="">Seleccionar Vehículo</option>
                                        @foreach($vehiculos as $v)
                                            <option value="{{ $v->id_vehiculos }}" 
                                                data-precio="{{ $v->precio_dia }}" 
                                                {{ $vehiculo->id_vehiculos == $v->id_vehiculos ? 'selected' : '' }}>
                                                {{ $v->marca }} {{ $v->modelo }} ({{ $v->precio_dia }}€/día)
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="fecha_inicio_{{ $index }}" class="form-label">Fecha de Inicio</label>
                                        <input type="date" class="form-control fecha-inicio" id="fecha_inicio_{{ $index }}" name="fecha_inicio[]" value="{{ $vehiculo->fecha_ini }}" required onchange="calcularPrecio({{ $index }})">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="fecha_fin_{{ $index }}" class="form-label">Fecha de Fin</label>
                                        <input type="date" class="form-control fecha-fin" id="fecha_fin_{{ $index }}" name="fecha_fin[]" value="{{ $vehiculo->fecha_final }}" required onchange="calcularPrecio({{ $index }})">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="alert alert-info precio-info" id="precio-info-{{ $index }}">
                ...
                <div class="alert alert-info mt-4" id="precio-total" data-precio="{{ $reserva->total_precio }}">
                    <strong>Precio Total:</strong> {{ number_format($reserva->total_precio, 2) }} €
                </div>
    ...
    <div class="row mb-3">
        <div class="col-md-12">
            ...
            <select class="form-select" id="vehiculos_${newIndex}" name="vehiculos[]" required onchange="calcularPrecio(${newIndex})">
                <option value="">Seleccionar Vehículo</option>
                @foreach($vehiculos as $v)
                    <option value="{{ $v->id_vehiculos }}" data-precio="{{ $v->precio_dia }}">
                        {{ $v->marca }} {{ $v->modelo }} ({{ $v->precio_dia }}€/día)
                    </option>
                @endforeach
            </select>
...
<script src="{{ asset('js/admin-edit-reserva.js') }}"></script>
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Pasar la plantilla y los valores iniciales al contenedor
        container.setAttribute('data-template', template);
        container.setAttribute('data-count', '{{ count($reserva_vehiculos) }}');
        
        // Configurar el precio total inicial
        document.getElementById('precio-total').setAttribute('data-precio', '{{ $reserva->total_precio }}');
    });
</script>
@endsection
